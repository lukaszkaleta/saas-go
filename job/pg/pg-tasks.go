...
func (pgTasks *PgTasks) MostUsedClient(ctx context.Context) ([]job.Job, error) {
	query := JobSelect() + `
		where action_created_by_id = (
			select action_created_by_id
			from task
			where user_id = @userId
			group by action_created_by_id
			order by count(*) desc
			limit 1
		)
		and 
	...
	return pgx.CollectRows(rows, MapJob(pgTasks.db))
}
