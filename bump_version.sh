...
# --- CONFIGURATION ---
VERSION_FILE="VERSION"   # file storing the version string (e.g., v0.2.104)

# --- READ CURRENT VERSION ---
if [[ ! -f "$VERSION_FILE" ]]; then
  echo "VERSION file not found. Create one with initial version like: v0.2.104"
  exit 1
fi
...
if [[ ! $CURRENT_VERSION =~ ^v([0-9]+)\.([0-9]+)\.([0-9]+)$ ]]; then
  echo "Invalid version format in VERSION file. Expected: v0.2.104"
  exit 1
fi
