...
func (p *PgContent) Update(ctx context.Context, model *universal.ContentModel) error {
	// Update name and content value on the owning table
	query := "update content set name_value = $1, name_slug = $2, value = $3 where id = $4"
	_, err := p.Db.Pool.Exec(ctx, query, model.Name.Value, model.Name.Slug, model.Value, p.Id)
	if err != nil {
		return err
	}
	return nil
...
func (p *PgContent) Model(ctx context.Context) *universal.ContentModel {
