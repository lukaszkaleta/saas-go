...
type PgChatUserInbox struct {
	db *pg.PgDb
}

func NewPgChatUserInbox(db *pg.PgDb) *PgChatUserInbox {
	return &PgChatUserInbox{db: db}
}

func (p *PgChatUserInbox) Last(ctx context.Context) ([]chat.Message, error) {
...
WITH latest_messages AS (
    ...
    JOIN job_message jm
        ON jm.chat_id = jc.id
    WHERE jc.worker_id = @currentUserId
...
func (p *PgChatUserInbox) CountUnread(ctx context.Context) (int, error) {
	currentUserId := universal.CurrentUserId(ctx)
